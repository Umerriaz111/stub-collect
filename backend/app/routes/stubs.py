...
def get_stubs():
    ...
    stubs = Stub.query.options(joinedload(Stub.listings)).filter_by(user_id=current_user.id).order_by(Stub.created_at.desc()).all()
    return jsonify({
        'status': 'success',
...
def get_stub(stub_id):
    """Get a specific stub"""
    stub = Stub.query.options(joinedload(Stub.listings)).filter_by(id=stub_id, user_id=current_user.id).first()
