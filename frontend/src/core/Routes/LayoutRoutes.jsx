...
const currentRouteHasAside = () => {
    ...
    const currentRoute = routes.find((route) => matchPath(route.path, currentPath))
...
export const PagesRoutes = () => {
    ...
    return (
        <Grid item xs={isSmallScreen ? 12 : hasAside ? 10.4 : 12}>
            <Routes>
                {routes.map((route, index) => (
                    <Route key={'pageRoute-' + index} path={route.path} element={route.page} />
                ))}
            </Routes>
        </Grid>
    )
...
export const HeaderRoutes = () => {
    return (
        <Routes>
            {routes.map((route, index) => (
                <Route key={'headerRoute-' + index} path={route.path} element={route.header || <></>} />
            ))}
        </Routes>
    )
}
...
export const FooterRoutes = () => {
    return (
        <Routes>
            {routes.map((route, index) => (
                <Route key={'footerRoute-' + index} path={route.path} element={route.footer || <></>} />
            ))}
        </Routes>
    )
}
